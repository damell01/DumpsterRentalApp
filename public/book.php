        ...
        <div class="book-card">
            ...
            <div class="unit-grid">
                ...
                <label class="unit-card<?= ((int)$u['id'] === $preselect_unit_id) ? ' selected' : '' ?><?= $isUnavailable ? ' unavailable' : '' ?>"
                    ...
                    // Show whichever rate is set, in priority order: Monthly,
                    // then Weekly, then Daily. Base Price is the last resort —
                    // only shown when none of the other three are configured.
                    if ($mr > 0):
                    ?>
                    ...
                    <?php elseif ($bp > 0): ?>
                    <div class="unit-rate">$<?= number_format($bp, 2) ?><?= $rd > 0 ? ' / ' . $rd . 'd' : '' ?></div>
                    <?php if ($ep > 0): ?>
                    ...
                    if (($mr > 0 || $wr > 0) && $dr > 0) $secondary[] = '$' . number_format($dr, 2) . '/day';
                    if (!empty($secondary)):
                    ?>
